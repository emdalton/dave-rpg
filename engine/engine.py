...
class GameEngine:
    ...
    def _process_turn(
        ...
        # location, resolve the path now before building the Pass 2 packet.
        # ------------------------------------------------------------------
        if (
            ...
            # Only invoke pathfinding when the target is not directly adjacent.
            # Single-step moves (adjacent or same location) go through Pass 2
            # as normal with the LLM issuing the location_change.
            if (
                target_id != current_loc
                and not self.db.is_location_connected(current_loc, target_id)
            ):
                route = self._resolve_multistep_move(target_id)
                action_record["route"] = route

                if not route["reachable"]:
                    # No valid path — return a canned response without an LLM call.
                    logger.info(
    ...
    def _apply_outcome(self, action_record: dict, outcome: dict) -> None:
        ...
        logger.info(
            "Outcome applied: type=%s attitudes=%d states=%d moves=%d items=%d details=%d",
            outcome.get("outcome_type"),
