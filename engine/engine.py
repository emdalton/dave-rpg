    ...
    def _apply_outcome(self, action_record: dict, outcome: dict) -> None:
        ...
        if loc_change:
            try:
                ...
                # Validate that the target location actually exists in the DB
                # before applying the move. This prevents the LLM from sending
                # a character to a location_id it hallucinated from prose context.
                if self.db.get_location(new_loc_id) is None:
                    logger.warning(
                        "Ignoring location_change to non-existent location_id=%d "
                        "(LLM may have inferred location from prose rather than DB state)",
                        new_loc_id,
                    )
                else:
